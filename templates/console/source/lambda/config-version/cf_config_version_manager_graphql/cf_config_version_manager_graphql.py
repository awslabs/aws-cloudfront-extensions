...
def manager_version_diff(
    distribution_id: str = "", version1: str = "", version2: str = ""
):
...
def get_version_diff(s3_client, dist_id, version1, version2):
    ...
    response = ddb_table.get_item(
        Key={"distributionId": dist_id, "versionId": int(version1)}
    )
    data = response["Item"]
    s3_bucket = data["s3_bucket"]
    s3_key1 = data["s3_key"]
    response = ddb_table.get_item(
        Key={"distributionId": dist_id, "versionId": int(version2)}
    )
    data = response["Item"]
    s3_key2 = data["s3_key"]
    # s3_client = boto3.client('s3')
    ...
    cmd = [
        ...
        "diff",
        "--no-index",
        local_config_file_name_version1,
        local_config_file_name_version2,
        ">/tmp/diff.txt",
        ";",
        "exit 0",
    ]
...
def manager_snapshot_diff(
    distribution_id: str = "", snapshot1: str = "", snapshot2: str = ""
):
...
def get_snapshot_diff(s3_client, distribution_id, snapshot1, snapshot2):
    ...
    response = ddb_snapshot_table.get_item(
        Key={"distributionId": distribution_id, "snapShotName": snapshot1}
    )
    snapshot_resp = response["Item"]
    if not snapshot_resp:
        raise Exception(
            f"Failed to get the snapshot with distribution id:{distribution_id}, snapshot_name:{snapshot1}"
        )
    src_version_1 = snapshot_resp["versionId"]
    response = ddb_snapshot_table.get_item(
        Key={"distributionId": distribution_id, "snapShotName": snapshot2}
    )
    snapshot_resp = response["Item"]
    if not snapshot_resp:
        raise Exception(
            f"Failed to get the snapshot with distribution id:{distribution_id}, snapshot_name:{snapshot2}"
        )
    ...
    ddb_table = ddb_client.Table(DDB_VERSION_TABLE_NAME)
    response = ddb_table.get_item(
        Key={"distributionId": dist_id, "versionId": int(version_1)}
    )
    data = response["Item"]
    s3_bucket = data["s3_bucket"]
    s3_key1 = data["s3_key"]
    response = ddb_table.get_item(
        Key={"distributionId": dist_id, "versionId": int(version_2)}
    )
    data = response["Item"]
    s3_key2 = data["s3_key"]
...
def manager_version_apply_config(
    src_distribution_id: str = "",
    target_distribution_ids: [str] = [],
    version: str = "",
):
    s3_client = boto3.client("s3")
    cf_client = boto3.client("cloudfront")
    return apply_config_version(
        s3_client, cf_client, src_distribution_id, version, target_distribution_ids
    )


def apply_config_version(
    s3_client, cf_client, source_dist_id, src_version, target_dist_ids
):
    # get specific cloudfront distributions version info
    ddb_client = boto3.resource("dynamodb")
    ddb_table = ddb_client.Table(DDB_VERSION_TABLE_NAME)
    response = ddb_table.get_item(
        Key={"distributionId": source_dist_id, "versionId": int(src_version)}
    )
    data = response["Item"]
    s3_bucket = data["s3_bucket"]
    s3_key1 = data["s3_key"]

    local_config_file_name_version = (
        "/tmp/" + source_dist_id + "_" + src_version + ".json"
    )
    ...
    with open(local_config_file_name_version) as config_file:
        ...
        for distribution_id in target_dist_ids:
            ...
            prev_config = cf_client.get_distribution_config(Id=distribution_id)
            etag = prev_config["ETag"]
            target_dist_caller_reference = prev_config["DistributionConfig"][
                "CallerReference"
            ]
            ...
            if dictData == prev_config["DistributionConfig"]:
                logger.info(
                    "the two configuration is same, no need to create a new version"
                )
            else:
                ...
                response = cf_client.update_distribution(
                    DistributionConfig=dictData, Id=distribution_id, IfMatch=etag
                )
...
def manager_snapshot_apply_config(
    ...
    src_version = get_snapshot_version(
        ddb_snapshot_table, snapshot_name, source_dist_id
    )
    ...
    local_config_file_name_version = download_version_config(
        ddb_table, source_dist_id, src_version
    )
    ...
    apply_distribution_from_local_config(
        cf_client,
        ddb_latest_table,
        local_config_file_name_version,
        src_snapshot,
    ...
    return {"statusCode": 200, "body": "succeed apply snapshot to target distributions"}


def apply_distribution_from_local_config(
    cf_client,
    ddb_latest_table,
    local_config_file_name_version,
    src_snapshot,
        ...
        for distribution_id in target_dist_ids:
            ...
            if dictData == prev_config["DistributionConfig"]:
                ...
                response = ddb_latest_table.update_item(
                    ...
                    ExpressionAttributeValues={":r": src_snapshot},
...
def download_version_config(ddb_table, source_dist_id, src_version):
    ...
    s3_client = boto3.client("s3")
    local_config_file_name_version = (
        "/tmp/" + source_dist_id + "_" + str(src_version) + ".json"
    )
    s3_client.download_file(s3_bucket, s3_key1, local_config_file_name_version)
...
def get_snapshot_version(ddb_snapshot_table, snapshot_name, source_dist_id):
    response = ddb_snapshot_table.get_item(
        Key={"distributionId": source_dist_id, "snapShotName": snapshot_name}
    )
    snapshot_resp = response["Item"]
    if not snapshot_resp:
        raise Exception(
...
def manager_version_config_tag_update(
    distribution_id: str = "", note: str = "", version: str = ""
):
    dist_id = distribution_id
    version_id = version
    dist_note = note
...
def update_config_version_note(ddb_table, dist_id, dist_note, version_id):
    response = ddb_table.update_item(
        Key={"distributionId": dist_id, "versionId": int(version_id)},
...
def manager_snapshot_config_tag_update(
    distribution_id: str = "", note: str = "", snapshot_name: str = ""
):
    dist_id = distribution_id
...
def update_snapshot_note(ddb_table, dist_id, dist_note, snapShotName):
    response = ddb_table.update_item(
        Key={"distributionId": dist_id, "snapShotName": snapShotName},
        UpdateExpression="set note = :r",
        ExpressionAttributeValues={":r": dist_note},
        ReturnValues="UPDATED_NEW",
    )
...
def get_all_reallog_list():
    cloudfront = boto3.client("cloudfront")
    dist_list = []
    response = cloudfront.list_realtime_log_configs()
    reallog_list = response["RealtimeLogConfigs"]["Items"]
    dist_list.extend(reallog_list)
    while "NextMarker" in response["RealtimeLogConfigs"]:
        next_token = response["RealtimeLogConfigs"]["NextMarker"]
        response = cloudfront.list_distributions(Marker=next_token)
        dist_list.extend(response["RealtimeLogConfigs"]["Items"])
    return dist_list
...
def manager_version_config_cf_list():
    ...
    reallog_list = get_all_reallog_list()
    ddb_client = boto3.resource("dynamodb")
    ...
    for dist in cf_dist_list:
        ...
        if "RealtimeLogConfigArn" in realLogArn:
            ...
            if len(reallog_list) > 0:
                samplingRateList = [
                    data
                    for data in reallog_list
                    if data.get("ARN") == tmp_dist["realLogArn"]
                ]
                if len(samplingRateList) > 0:
                    tmp_dist["logSamplingRate"] = samplingRateList[0]["SamplingRate"]
...
def get_config_version_s3_link(ddb_table, dist_id, versionId):
    response = ddb_table.get_item(
        Key={"distributionId": dist_id, "versionId": int(versionId)}
    )
    data = response["Item"]
...
def manager_snapshot_get_content(distribution_id: str = "", snapshot_name: str = ""):
    ...
    ddb_snapshot_table = ddb_client.Table(DDB_SNAPSHOT_TABLE_NAME)
    response = ddb_snapshot_table.get_item(
        Key={"distributionId": distribution_id, "snapShotName": snapshot_name}
    )
    ...
    response = ddb_table.get_item(
        Key={"distributionId": distribution_id, "versionId": int(src_version)}
    )
    data = response["Item"]
    if not data:
        raise Exception(
            f"Failed to get the version with distribution id:{distribution_id}, version_id:{src_version}"
        )
...
def get_all_version(ddb_table, dist_id):
    response = ddb_table.query(
        KeyConditionExpression=Key("distributionId").eq(dist_id), ScanIndexForward=False
    )
    data = response["Items"]
    return data
...
def get_all_snapshot(ddb_client, ddb_snapshot_table, dist_id):
    ...
    for snap_shot_record in snapshot_response:
        tmp = {
            "id": snap_shot_record["snapShotName"],
            "distribution_id": snap_shot_record["distributionId"],
            "snapshot_name": snap_shot_record["snapShotName"],
            "note": snap_shot_record["note"],
            "dateTime": snap_shot_record["dateTime"],
        }

        # query more info from version table
        response = ddb_version_table.get_item(
            Key={"distributionId": dist_id, "versionId": snap_shot_record["versionId"]}
        )
...
def createVersionSnapShot(
    distributionId: str = "", snapShotName: str = "", snapShotNote: str = ""
):
    if distributionId == "":
    ...
    return create_snapshot(
        ddb_table, distributionId, latest_version, snapShotName, snapShotNote
    )
...
def create_snapshot(
    ...
    return {"statusCode": 200, "body": "succeed create new snapshot"}
...
def get_latest_version(ddb_client, distributionId):
    ...
    if len(record_list) == 0:
        raise Exception(
            "There is no latest version for distributionId:" + distributionId
        )
    latest_version = record_list[0]["versionId"]
    logging.info(
        "The latest version of distribution:"
        + str(distributionId)
        + " is "
        + str(latest_version)
    )
...
def check_snapshot_name_duplication(ddb_table, distributionId, snapShotName):
    response = ddb_table.get_item(
        Key={"distributionId": distributionId, "snapShotName": snapShotName}
    )
...
def delete_snapshot(ddb_table, distributionId, snapShotName):
    ...
    logger.info(response)
    return {"statusCode": 200, "body": "succeed delete snapshot"}
