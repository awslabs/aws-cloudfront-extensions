...
def test_construct_query_string():
    ...
    assert query_string == 'SELECT count(timestamp), \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 group by \"cs-host\";'

    metric = 'requestOrigin'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT count(timestamp), \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 AND (\"x-edge-detailed-result-type\" = \'Miss\' OR (\"x-edge-detailed-result-type\" like \'%Origin%\' AND \"x-edge-detailed-result-type\" <> \'OriginShieldHit\')) group by \"cs-host\";'

    metric = 'statusCode'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT \"sc-status\", count(timestamp), \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 GROUP BY \"sc-status\", \"cs-host\";'

    metric = 'statusCodeOrigin'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT \"sc-status\", count(timestamp), \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 AND (\"x-edge-detailed-result-type\" = \'Miss\' OR (\"x-edge-detailed-result-type\" like \'%Origin%\' AND \"x-edge-detailed-result-type\" <> \'OriginShieldHit\')) group by \"sc-status\", \"cs-host\";'

    metric = 'bandwidth'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT sum(\"sc-bytes\")/(60*0)*8, \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 group by \"cs-host\";'

    metric = 'bandwidthOrigin'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT sum(\"sc-bytes\")/(60*0)*8, \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 AND (\"x-edge-detailed-result-type\" = \'Miss\' OR (\"x-edge-detailed-result-type\" like \'%Origin%\' AND \"x-edge-detailed-result-type\" <> \'OriginShieldHit\')) group by \"cs-host\";'

    metric = 'chr'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT cast((sum(case when \"x-edge-result-type\" like \'%Hit\' then 1 else 0 end) * 100.0 / count(1)) as decimal(38,2)) as ratio, \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 AND \"x-edge-response-result-type\" <> \'LimitExceeded\' AND \"x-edge-response-result-type\" <> \'CapacityExceeded\' group by \"cs-host\";'

    metric = 'chrBandWidth'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT cast((sum(case when \"x-edge-result-type\" like \'%Hit\' then \"sc-bytes\" else 0 end)*100.0/(60*5)*8) / (sum(\"sc-bytes\")/(60*5)*8 ) as decimal(38,2)) as ratio, \"cs-host\" FROM \"TESTDB\".\"TESTTABLE\" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 AND \"x-edge-response-result-type\" <> \'LimitExceeded\' AND \"x-edge-response-result-type\" <> \'CapacityExceeded\' group by \"cs-host\";'
    ...
    metric = 'latencyratio'
    query_string = construct_query_string(db_name, date_time_str_start, date_time_str_end, metric, table_name, m_interval, latency_limit)
    assert query_string == 'SELECT cast((sum(case when "time-taken" >= 0 then 1 else 0 end) * 100.0 / count(*)) as decimal(38,2)) as ratio, "cs-host" FROM "TESTDB"."TESTTABLE" WHERE year = 2022 AND month = 1 AND day = 21 AND hour = 8 AND minute = 15 AND timestamp <= 1642724127.0 AND timestamp > 1642724127.0 group by "cs-host";'
